...
/**
 *
 * Class that generates gremlin statements to push data from task into the service.
 *
 *  @author olignat
 */
final class GremlinQueryBuilder {
    ...
    static String build(String traversal, SinkRecord record)
    {
    	...
    	if (record != null) {
    		processedTraversal = handleSchemaType(
				 handleSchemaType(processedTraversal, 
						 record.keySchema(), 
						 record.key(), 
						 GremlinQueryBuilder.KEY_DESIGNATION), 
				 record.valueSchema(), 
				 record.value(), 
				 GremlinQueryBuilder.VALUE_DESIGNATION);
    	}
    ...
    private static String handleSchemaType(String traversal, Schema schema, Object value, String prefix) {
        ...
        log.debug("Processing query build for prefix '{}', schema type '{}' and value '{}'", prefix, schema != null ? schema.type() : "<null schema>", value);
        ...
        if (schema != null) {
        	if (schema.type() == Type.MAP) {
	        	...
	        	for (Map.Entry<?, ?> entry : valueMap.entrySet()) {
	        		...
	        		updatedTraversal = updatedTraversal.replace("${" + prefix + "." + entry.getKey() + "}", mapValue == null ? "" : mapValue.toString());
	            }        
	    	}
	            ...
	            for (Field field : schema.fields()) {
	            	...
	            	updatedTraversal = updatedTraversal.replace("${" + prefix + "." + field.name() + "}", fieldValue == null ? "" : fieldValue.toString());
	            }
	        }
	            ...
	            for (int i = 0; i < valueArray.size(); i++) {
	            	...
	            	updatedTraversal = updatedTraversal.replace("${" + prefix + "[" + i + "]}", arrayValue == null ? "" : arrayValue.toString());
	            }
	        }
        }
