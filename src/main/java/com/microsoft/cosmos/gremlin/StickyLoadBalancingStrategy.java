...
/**
 * In Microsoft Azure Cosmos DB there is only a single host (DNS CNAME) 
 * that points to a single VIP behind which there are many physical nodes serving traffic.
 * It is incorrect to mark host as unavailable and look for a better one because there won't be a better one.
 * VIP does not go down, but a single node can.
 * Failure of a node should not render entire cluster unusable
 * We need load balancing strategy that sticks to a single host no matter what.
 * 
 *  * @author olignat
 *
 */
